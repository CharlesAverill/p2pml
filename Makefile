...
run: build
	$(OPAM_EXEC) $(DUNE) exec -- p2pml example_adj.txt

debug: build
	$(OPAM_EXEC) ocamldebug _build/default/p2pml/main.bc

DOCS_PATH=docs/
DOCS_NAME=p2pml
DOCS_DESCR=Distributed filesharing protocol
...
<meta content="https://github.com/CharlesAverill/p2pml" property="og:url" />
endef
...
docs: cleandocs build
	...
	mv $(DOCS_PATH)p2pml/p2pml.html $(DOCS_PATH)index.html
	mv $(DOCS_PATH)p2pml $(DOCS_PATH)module
